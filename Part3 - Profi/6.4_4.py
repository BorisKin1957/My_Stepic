'''Вам доступен именованный кортеж User, который содержит данные о пользователе некоторого
ресурса. Первым элементом именованного кортежа является имя пользователя, вторым — фамилия,
...
new = sorted(users, key=lambda x: x.email)

plans = {0: 'Gold', 1:'Silver', 2: 'Bronze', 3: 'Basic'}

for i in range(4):
    for item in new:
        if item.plan == plans[i]:
