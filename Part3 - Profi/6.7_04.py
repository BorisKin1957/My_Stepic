...
1088+1091+1095+1082+1072=5428UCТимур составляет список покупок, но так как на его клавиатуре перестал работать блок
с цифрами, то вместо указания количества товара числом, он добавляет его в список столько раз, сколько планирует купить.
...
products = sorted(Counter(input().split(',')).items())
lenght = len(max(products, key=lambda x: len(x[0]))[0])

for tovar, count in products:
    product = tovar.replace(' ', '')
    counter = Counter(product)
    price = defaultdict(int)
    for key, value in counter.items():
        price[product] += int(ord(key)) * value
    total = price[product] * count
    print(f'{tovar}{" " * (lenght - len(tovar))}: {price[product]} UC x {count} = {total} UC')