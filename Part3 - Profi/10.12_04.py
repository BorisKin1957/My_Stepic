...
if min([item.mass for item in items]) > netto:
    print('Рюкзак собрать не удастся')
else:
    new = [(item.name, item.mass, item.price) for item in items]
    s = set()

    for i in range(1, len(new) + 1):
        for sing in combinations(new, r=i):
            if sum(map(lambda x: x[1], sing)) <= netto:
                s.add(sing)

    tmp = 0
    for i in s:
        volume = sum(map(lambda x: x[2], i))
        if volume > tmp:
            tmp = volume
            result = i
    print(*[i[0] for i in sorted(result)], sep='\n')
