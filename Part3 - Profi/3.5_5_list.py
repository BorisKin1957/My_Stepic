...
На вход программе в первой строке подается натуральное число nn — количество сотрудников, работающих
в организации. В последующих nn строках вводятся данные о каждом сотруднике: имя, фамилия и дата рождения,
...
for _ in range(n):
    ...
    name_lib.append((name, dt))

name_lib = sorted(name_lib, key=lambda x: x[1])
old = name_lib[0][1]
for i in name_lib:
    if i[1] == old:
        count += 1
if count == 1:
    print(f'{old.strftime(pat)} {name_lib[0][0]}')
else:
    print(f'{old.strftime(pat)} {count}')
