...
Напишите программу, которая группирует все числа данной последовательности по сумме их цифр и
...
Программа должна сгруппировать все числа из натуральной последовательности от 1 до n по сумме
...
# ТУПО по подсказке решал. Суть: создаем словарь, в котором ключи - суммы цифр числа из списка.
# Высчитываем, сколько раз такая сумма может быть собрана из чисел в этом списке, и выводим максимум из значений словаря

def sum_num(num):
    return sum([int(i) for i in list(str(num))])


numbers = [int(i) for i in range(1, int(input()) + 1)]
result = {}

for num in numbers:
    result[sum_num(num)] = result.get(sum_num(num), 0) + 1
