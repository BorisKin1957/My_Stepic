...
и окончания первого занятия в формате HH:MM - HH:MM, на второй строке — время начала и окончания
...
pat = '%H:%M'
start = datetime.strptime(input(), pat)
end = datetime.strptime(input(), pat)

tm = start
while tm <= end - timedelta(minutes=45):
    print(f'{tm.strftime(pat)} - {(tm + timedelta(minutes=45)).strftime(pat)}')
    tm += timedelta(minutes=55)
