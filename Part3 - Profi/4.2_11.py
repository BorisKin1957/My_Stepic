...
Мы получили запись в привычном CSV формате, в котором в первом столбце указывается
имя объекта, а в последующих — значения соответствующих свойств этого объекта.
...
Реализуйте функцию condense_csv(), которая принимает два аргумента в следующем формате:
    ...
    аналогичен формату текстового фрагмента, рассмотренного в условии задачи:
    каждая строка файла содержит три значения через запятую, а именно имя объекта,
    свойство этого объекта, значение свойства; все объекты имеют равные свойства
    и в равных количествах
    id_name — общее название для объектов

Функция должна привести содержимое файла в привычный CSV формат, сгруппировав
строки по первому столбцу и назвав первый столбец id_name. Полученный результат
функция должна записать в файл condensed.csv.
...
def condense_csv(file, id_name):
    with open(file, encoding='utf-8') as file_in, open('condensed.csv', 'w', encoding='utf-8') as file_out:
        lib = {}
        rows = list(csv.reader(file_in))
        file_out.write(id_name)
        for id_name, property, value in rows:
            if id_name not in lib:
                lib[id_name] = [{property: value}]
            else:
                lib[id_name].append({property: value})

        for i_lib in lib[list(lib.keys())[0]]:
            for key in i_lib.keys():
                file_out.write(f',{key}')
        file_out.write('\n')

        for key, value in lib.items():
            file_out.write(f'{key}')
            for i_lib in value:
                for value in i_lib.values():
