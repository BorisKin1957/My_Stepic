...
На вход программе подается число nn — количество строк в базе данных о
продажах интернет-магазина. Далее следует nn строк с записями вида покупатель
товар количество, где покупатель — имя покупателя (строка без пробелов),
товар — название товара (строка без пробелов), количество —
...
приобретенных им товаров в лексикографическом порядке, после названия каждого товара —
...
for _ in range(int(input())):
    name, tov, kol = input().split()
    base.setdefault(name, {})
    base[name][tov] = base[name].get(tov, 0) + int(kol)

for name, value in sorted(base.items()):
    print(name, ':', sep='')
    for tov, kol in sorted(value.items()):
        print(tov, kol)
...
#     nam, tov, kol = input().split()
#     if nam in base:
#         if tov in base[nam]:
#             base[nam][tov] += int(kol)
#         else:
#             base[nam][tov] = int(kol)
#     else:
#         base[nam] = base.get(nam, {tov: int(kol)})
#
# for nam, value in sorted(base.items()):
#     print(nam, ':', sep='')
#     for tov, kol in sorted(value.items()):
#         print(tov, kol)