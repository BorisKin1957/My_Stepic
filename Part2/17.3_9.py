...
Она должна возвращать список словарей, интерпретируя первую строку
как имена ключей, а каждую последующую строку как значения этих ключей.
...
def read_csv():
    with open('data.csv') as file:
        key_l = file.readline().strip().split(',')
        csv_list = []
        for line in file:
            value_l = line.strip().split(',')
            sl = {}
...
'''ПРАВИЛЬНО следовало создавать словарь через ZIP 
о такой функции я и не вспомнил. 
РЕШЕНИЕ:
