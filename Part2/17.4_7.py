...
всех пользователей (не меняя порядка следования), которые были в сети не менее часа.
...
with open('logfile.txt', encoding='utf-8') as f1, open('output.txt', 'w', encoding='utf-8') as f2:
    for line in f1:
        l = line.rstrip().split()
        t0 = int(l[2][:2]) * 60 +  int(l[2][-3:-1])
        t1 = int(l[3][:2]) * 60 +  int(l[3][-2:])
        if (t1 - t0) >= 60:
            print(l[0], l[1][:-1], file=f2)