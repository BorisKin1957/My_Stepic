...
чтобы он вывел данный словарь, расположив его элементы по следующему правилу: первый,
последний, второй, предпоследний, третий, и так далее.
...
data = OrderedDict({'Name': 'Брусника', 'IsNetObject': 'да', 'OperatingCompany': 'Брусника',
                    ...
                    'SeatsCount': '10'})
new = list(data)
data.move_to_end(new[0])
for i in range(1, int(len(new) / 2) + 1):
    data.move_to_end(new[-i])
    data.move_to_end(new[i])
