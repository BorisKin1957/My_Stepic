...
Программа должна определить минимальное и максимальное значения функции f(x) на отрезке [a;b]
в целых точках и вывести полученный результат в следующем формате:
...
s, l = input(), input().split()

a, b = int(l[0]), int(l[1])

l = [eval(s) for x in range(a, b + 1)]

print(f'Минимальное значение функции {s} на отрезке [{a}; {b}] равно {min(l)}')
print(f'Максимальное значение функции {s} на отрезке [{a}; {b}] равно {max(l)}')