...
число n — количество сотрудников, работающих в организации. В последующих nn строках вводятся данные о каждом сотруднике:
...
pat = '%d.%m.%Y'
today = datetime.strptime(input(), pat)
...
new_lib = sorted(name_lib.items(), key=lambda x: x[0], reverse=True)

name = ''
for i in new_lib:
    d = i[0].replace(year=today.year)       # замена года в дате рождения на текущий год
    d1 = i[0].replace(year=today.year + 1)  # замена года в дате рождения на следующий от текущего год
    if 0 < (d - today).days <= 7 or 0 < (d1 - today).days <= 7:
        name = i[1][0]
        break
if name:
    print(name)
