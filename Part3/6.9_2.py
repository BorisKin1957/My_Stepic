...
Примечание 1. Программа должна добавлять нужное количество пробелов, если один ингредиент
...
total, l = 0, 0
L = input().split(',')
lenght = max([len(i) for i in L])
all = sorted(Counter(L).items(), key=lambda x: x[0])

price = ChainMap(bread, meat, sauce, vegetables, toppings)

for choice, count in all:
    total += price[choice] * count
    stroke = f'{choice} {(lenght - len(choice)) * " "}x {count}'
    l = max([l, len(stroke)])
    print(stroke)
l = max([l, len(f'ИТОГ: {total}р')])
print(l * '-')
