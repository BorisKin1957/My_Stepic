...
Также ему нравится плавать по длинным дорожкам, поэтому из всех работающих в это время бассейнов нужно
...
good = []
for item in data:
    s = item['WorkingHoursSummer']['Понедельник']   # '10:00-11:00'
    open, close = int(s[:2]), int(s[-5:-3])         # open: 10 close: 11
    length = item['DimensionsSummer']['Length']     # length: 50
    width = item['DimensionsSummer']['Width']       # wight: 21
    if open <= 10 and close >= 12:
        good.append([length, width, item['Address']])

good = sorted(good, key=lambda x: (-x[0], -x[1]))

print(f'{good[0][0]}x{good[0][1]}\n{good[0][2]}')
