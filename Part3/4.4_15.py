...
Напишите программу, которая:
    ...
    определяет район Москвы, в котором находится больше всего заведений, и выводит
    название этого района и количество заведений в нем
...
for item in data:
    key1 = item['District']
    flag = item['IsNetObject']
    key2 = item['OperatingCompany']
    region[key1] = region.get(key1, 0) + 1
    if flag == 'да':
        net[key2] = net.get(key2, 0) + 1

max_area = max(region.items(), key=lambda x: x[1])
max_net = max(net.items(), key=lambda x: x[1])

print(f'{max_area[0]}: {max_area[1]}')
